...
def detect_mysql(host: str, port: int, timeout: float):
    try:
        s = tcp_connect(host, port, timeout)
        data = recv_some(s, timeout, 128)
        s.close()
        return bool(data and data[0] == 0x0A)
    except Exception:
        return False

def https_tls_probe(host: str, port: int, timeout: float, sni_name, alpn_list):
    ...
    try:
        s = tcp_connect(host, port, timeout)
        tls = ctx.wrap_socket(s, server_hostname=(sni_name if sni_name else None))
        ...
        cert_der = b""
        try:
            cert_der = tls.getpeercert(binary_form=True) or b""
        except Exception:
            cert_der = b""
...
def http_head_plain(host: str, port: int, timeout: float, host_header: str):
    try:
        ...
        s.sendall(req)
        raw = recv_until(s, timeout)
        s.close()
        code = parse_http_status_line(raw)
        if re.fullmatch(r"\d{3}", code or ""):
            return True, code
        return False, "-"
    except Exception:
        return False, "-"
...
def _ws_accept(key_b64: str):
    ...
    sha = hashlib.sha1(raw + GUID.encode()).digest()
...
# ===================== SNI 自动模式 =====================
def sni_candidates_from_cert(cert_info: dict, cn_hint):
    cands = []
...
def guess_url(proto: str, host_header: str, port: int):
    p = proto.upper()
    if p == "HTTPS":
        return f"https://{host_header}:{port}/"
    if p == "HTTP":
        return f"http://{host_header}:{port}/"
    if p == "WSS":
        return f"wss://{host_header}:{port}/"
    if p == "WS":
        return f"ws://{host_header}:{port}/"
    if p == "TLS-Other":
        return f"tls://{host_header}:{port}"
    return "-"
...
# ===================== 扫描核心 =====================
def scan_one(host: str, port: int, timeout: float, sni_mode: str, sni_value, ws_path: str):
    ok, bnr = detect_ssh(host, port, timeout)
    if ok:
        return row(host, port, "SSH", bnr)
...
def export_to_excel(all_rows, cats, summary, xlsx_path: str):
    try:
        ...
        for col_idx in range(1, 3):
            max_len = 12
            for row_ in ws_sum.iter_rows(min_col=col_idx, max_col=col_idx):
                for cell in row_:
        ...
        def write_sheet(title, rows):
            ...
            for col_idx, name in enumerate(header, 1):
                if rows:
                    max_len = max(len(str(name)), *(len(str(rr.get(name, ""))) for rr in rows))
                else:
                    max_len = len(name)
...
def export_to_csv(all_rows, cats, summary, out_base: str):
    with open(out_base + "-summary.csv", "w", newline="", encoding="utf-8") as f:
        ...
        w.writerow(["Total Ports", summary["total"]])
        w.writerow([])
        w.writerow(["By Proto", "Count"])
        for k, v in sorted(summary["by_proto"].items()):
            w.writerow([k, v])
        w.writerow([])
        w.writerow(["By TLS Version (HTTPS/WSS/TLS-Other)", "Count"])
        for k, v in sorted(summary["by_tls_ver"].items()):
            w.writerow([k, v])
        w.writerow([])
        w.writerow(["Proto", "Ports"])
        for k, v in summary["ports_by_proto"].items():
            w.writerow([k, " ".join(str(p) for p in v)])
...
def main():
    ...
    default_ports = "80,443,22,3306,6379,5432,8080,8443"

    print("=== 本机多协议端口扫描工具 ===")
    print(f"默认扫描目标：{default_host}")
    print(f"默认端口：{default_ports}")
    print("可直接回车使用默认值，也可通过命令行传参。")
    print('示例：python3 proto_scan.py 127.0.0.1 "1-1024"')
    print("")

    ap = argparse.ArgumentParser(
        description="Multi-protocol scanner with TLS & WebSocket; default target localhost; export all results"
    )
    ap.add_argument("host", nargs="?", default=None, help="host or ip; default 127.0.0.1")
    ap.add_argument("ports", nargs="?", default=None, help='port spec, e.g. "80-90,443,8080"')
    ...
    ap.add_argument("--out", type=str, default="scan_results_local", help="output base filename")
    args = ap.parse_args()

    if not args.host:
        tmp = input(f"请输入目标主机/IP（直接回车默认 {default_host}）: ").strip()
        args.host = tmp if tmp else default_host

    if not args.ports:
        tmp = input(f"请输入端口/范围（直接回车默认 {default_ports}）: ").strip()
        args.ports = tmp if tmp else default_ports

    if not args.host:
        print("错误：目标主机不能为空。", file=sys.stderr)
        sys.exit(1)

    ports = parse_port_spec(args.ports)
    if not ports:
        print("错误：端口格式无效。", file=sys.stderr)
        print('请使用类似格式："80,443" 或 "80-90,443,8080"', file=sys.stderr)
        sys.exit(1)
    ...
    print("")
    print(f"[+] 开始扫描：host={args.host}, ports={args.ports}, timeout={args.timeout}, workers={args.workers}")
    print("")
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as ex:
        ...
        for fut in as_completed(futs):
            try:
                ...
                print(f"[ERR] 扫描线程异常: {e}", file=sys.stderr)
                continue
    ...
    for k, plist_ in sorted(summary["ports_by_proto"].items()):
        plist = ", ".join(str(p) for p in plist_)
