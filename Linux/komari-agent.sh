...
# [修改] 默认强制使用指定的加速地址
github_proxy="https://github.newthbay.com"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --install-service-name)
            ...
            ;;
        --install-ghproxy)
            # 虽然保留参数解析，但后续逻辑会强制覆盖或优先使用硬编码的代理
            github_proxy="$2"
            shift 2
            ;;
        --install-version)
...
# [修改] 强制确保代理地址为指定地址 (覆盖可能通过命令行传入的其他参数)
github_proxy="https://github.newthbay.com"
...
log_config "  GitHub proxy: ${GREEN}$github_proxy${NC}"
...
# Download binary
# [修改] 移除直连判断，统一下载逻辑
log_step "Downloading $file_name via proxy (${github_proxy})..."
